...
- **마지막 커밋:** [AG] feat: zoom 제거 및 네이티브 85% 레이아웃 스케일링 (680px)

---

### 최근 작업 내용 (2026-03-08)

#### 1. TopBar 고정 및 레이아웃 교정
- **TopBar (`TopBar.style.js`)**: `position: fixed`로 고정. 800px 중심의 앱 규격이 85% 축소됨에 따라 `max-width: 680px`로 조정 및 중앙 정렬(`left: 50%`, `transform: translateX(-50%)`).
- **페이지 간격**: TopBar가 `fixed`로 전환됨에 따라 모든 페이지(`Checklist`, `Dashboard`, `Report`, `ProcessRepo`)의 루트 컨테이너에 `padding-top: 52px`를 일괄 적용하여 상단 바에 가려지지 않게 함.

#### 2. 체크리스트 스크롤 감지(Scroll Spy) 고도화
- **정밀 감지 (`Checklist.jsx`)**: `window.innerHeight`, `scrollHeight` 등을 `Math.max`로 교차 검증하여 브라우저 상관없이 정확한 바닥 감지 구현.
- **바닥 점프 해결**: 줌 배율 오차로 인해 마지막 항목('내부마감공사')이 선택되지 않던 문제 해결. 이제 바닥에 10px 이내로 닿으면 즉시 마지막 항목이 하이라이트됨.

#### 3. 네이티브 85% 스케일링 수술 (진행 중)
- **Zoom 제거 (`GlobalStyle.jsx`)**: 브라우저의 네이티브 수치(스크롤 높이 등)를 왜곡하던 `zoom: 0.85`를 삭제.
- **폰트 및 너비**: `html` 폰트 사이즈를 `13px`로 축소하고, 전체 앱 너비(`#root`)를 `680px`로 고정하여 줌 효과를 네이티브 수준에서 구현 시작.
- **남은 과제**: 체크리스트 사이드바의 `calc` 위치값 등 800px 기준으로 작성된 하드코딩 수치들을 680px 기준으로 전수 조사하여 보정 필요.
